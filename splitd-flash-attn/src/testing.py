# This file is copied from https://github.com/Dao-AILab/flash-attention/blob/main/flash_attn/cute/testing.py
import math
...
def generate_random_padding_mask(max_seqlen, batch_size, device, mode="random", zero_lengths=False, min_seqlen=None):
...
def attention_ref(
    ...
    if gather_kv_indices is not None:
        ...
        if topk_len < seqlen_k:
            topk_index_mask = torch.full((batch, seqlen_q, seqlen_k), False, device="cuda").scatter_(
